...
def propagateILM(volumeOriginal, segmentation, segmentation_original, saved_slices, rect_correction, mode):
    """ 
    ...
    #set BM 
    cropped_segmentation = np.where(cropped_segmentation != dictParameters['ILM_VALUE'], 0, cropped_segmentation)
    cropped_segmentation[:,:, 0]  = np.where(segmentation[rect_correction[2]-1:rect_correction[3]+1,: ,rect_correction[0] - 1] == dictParameters['ILM_VALUE'], dictParameters['ILM_VALUE'], 0)
    cropped_segmentation[:,:, -1] = np.where(segmentation[rect_correction[2]-1:rect_correction[3]+1,:,  rect_correction[1] + 1] == dictParameters['ILM_VALUE'], dictParameters['ILM_VALUE'], 0)
    cropped_segmentation[0 ,:,:]  = np.where(segmentation[rect_correction[2]-1,:,rect_correction[0]-1:rect_correction[1]+2] == dictParameters['ILM_VALUE'], dictParameters['ILM_VALUE'], 0)
    cropped_segmentation[-1,:,:]  = np.where(segmentation[rect_correction[3]+1,:,rect_correction[0]-1:rect_correction[1]+2] == dictParameters['ILM_VALUE'], dictParameters['ILM_VALUE'], 0)
    cropped_segmentation = np.swapaxes(cropped_segmentation, axis1=0, axis2=2)
    
    #connect points with lines
    cropped_segmentation = connectpoints(cropped_segmentation,mode)
    ...
    if(mode is 'high'):
        try:
            for i in range(1,cropped_segmentation.shape[0]-1):
                ...
                values = np.where(cropped_segmentation[i].transpose() == dictParameters['ILM_VALUE'])
                y_values = sp.medfilt(values[1],dictParameters['MAN_ILM_MEDIAN']).astype('uint16')
                result[y_values,values[0]] = dictParameters['ILM_VALUE']
    ...
    cropped_segmentation = np.where(segmentation_original[rect_correction[2]-1:rect_correction[3]+1,:,rect_correction[0]-1:rect_correction[1]+2] == dictParameters['BM_VALUE'], dictParameters['BM_VALUE'], cropped_segmentation).astype('uint8')
...
def connectpoints(cropped_segmentation,mode,dictParameters):
    ...
    for k in range (1,cropped_segmentation.shape[0]-1):
        points = np.asarray(np.where(cropped_segmentation[k].transpose() == dictParameters['ILM_VALUE'])).astype('int32')
        buffer_slice = cropped_segmentation[k].copy()
        
        for i in range (points[0].shape[0]-1):
            if(np.abs(points[0][i] - points[0][i+1]) > 1):
                if(mode is 'low'):
                    cv2.line(buffer_slice,(points[0][i],points[1][i]),(points[0][i+1],points[1][i+1]),dictParameters['ILM_VALUE'],1)
                else:
                    cv2.line(buffer_slice,(points[0][i],points[1][i]),(points[0][i+1],points[1][i+1]),dictParameters['ILM_VALUE'], dictParameters['MAN_ILM_THICKNESS'])
                buffer_slice[:,points[0][i]] = 0
                buffer_slice[points[1][i],points[0][i]] = dictParameters['ILM_VALUE']
        
        buffer_slice[:,points[0][-1]] = 0
        buffer_slice[points[1][-1],points[0][-1]] = dictParameters['ILM_VALUE']
...
def getGraph(_slice, shortest_path,dictParameters):
    ...
    slice_copy = cv2.bilateralFilter(slice_copy,3,2,2)
    # Gradient filter with kernel width 3 to enforce smoothness
    grad = cv2.filter2D(slice_copy,-1, np.array([[-1],[1]]))
    grad = np.where(grad > 0 , grad, 0)

    p2, p98 = np.percentile(grad, (2, 98))
    grad = exposure.rescale_intensity(grad, in_range=(p2, p98))

    # allow only areas where a line was inpainted
    buffer_result[:,1:-1] = np.where(buffer_result[:,1:-1] == dictParameters['ILM_VALUE'], grad, np.nan)
    buffer_result[:,0] = -2
    buffer_result[:,-1] = 2
